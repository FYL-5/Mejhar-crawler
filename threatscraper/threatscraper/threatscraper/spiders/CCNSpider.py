...
class CCNSpider(scrapy.Spider):
    name = 'CCNCert'
    start_urls = ['https://www.ccn-cert.cni.es/en/updated-security/warnings-ccn-cert.html']
    prefix_url = 'https://www.ccn-cert.cni.es'
    startpage=0

    def parse(self, response):
        for link in response.css('td.list-title a::attr(href)'):
